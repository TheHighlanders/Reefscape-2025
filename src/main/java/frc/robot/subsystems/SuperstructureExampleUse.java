...
public class SuperstructureExampleUse extends SubsystemBase {
  ...
  public void runTests() {
    ...
    Commands.print("Forward Exclusion Test: " +
        (StateRequest.getCurrentState(FirstEnumState.class) == FirstEnumState.ONE ? "PASSED" : "FAILED")).schedule();

    StateRequest.create(FirstEnumState.THREE);
    StateRequest.create(FirstEnumState.ONE);
    Commands.print("Reverse Direction Test: " +
        (StateRequest.getCurrentState(FirstEnumState.class) == FirstEnumState.ONE ? "PASSED" : "FAILED")).schedule();

    var stateChanged = StateRequest.createStateChangeSupplier(FirstEnumState.class);
    StateRequest.create(FirstEnumState.TWO);
    Commands.print("State Change Test: " +
        (stateChanged.getAsBoolean() ? "PASSED" : "FAILED")).schedule();

    var stateChanged2 = StateRequest.createStateChangeSupplier(FirstEnumState.class);
    StateRequest.create(FirstEnumState.TWO);
    Commands.print("State Change Test: " +
        (stateChanged2.getAsBoolean() ? "PASSED" : "FAILED")).schedule();
    ...
    Commands.print("Cross-Enum Exclusion Test: " +
        (StateRequest.getCurrentState(SecondEnumState.class) == SecondEnumState.A ? "PASSED" : "FAILED")).schedule();
